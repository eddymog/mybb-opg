...
if($mybb->input['action'] == "do_newreply" && $mybb->request_method == "post")
{
	...
	// Límite de 1000 palabras para temas con prefijo "Diario"
	if(!$post['savedraft'] && $thread['prefix'] != 0)
	{
		$thread_prefix_data = build_prefixes($thread['prefix']);
		if(is_array($thread_prefix_data) && strtolower($thread_prefix_data['prefix']) === 'diario')
		{
			$mensaje_limpio = preg_replace('/\[.*?\]/s', ' ', $mybb->get_input('message'));
			$num_palabras = count(preg_split('/\s+/u', trim($mensaje_limpio), -1, PREG_SPLIT_NO_EMPTY));
			if($num_palabras > 1000)
			{
				$post_errors[] = 'Los temas con prefijo <strong>Diario</strong> tienen un límite de <strong>1.000 palabras</strong>. Tu mensaje contiene '.$num_palabras.' palabras.';
			}
		}
	}
...
if($mybb->input['action'] == "newreply" || $mybb->input['action'] == "editdraft")
{
	...
	eval("\$newreply = \"".$templates->get("newreply")."\";");
        ...
        // Contador de palabras en tiempo real para temas con prefijo "Diario"
        if($thread['prefix'] != 0)
        {
                $tp = build_prefixes($thread['prefix']);
                if(is_array($tp) && strtolower($tp['prefix']) === 'diario')
                {
                        $newreply .= '<script>(function(){'
                                .'var LIMIT=1000,ta=document.getElementById("message");'
                                .'if(!ta)return;'
                                .'var ctr=document.createElement("p");'
                                .'ctr.id="opg_diario_ctr";'
                                .'ctr.style.cssText="margin:4px 0 0;font-size:12px;text-align:right;transition:color .2s;";'
                                .'ta.parentNode.insertBefore(ctr,ta.nextSibling);'
                                .'function cW(t){var s=t.replace(/\\[[\\s\\S]*?\\]/g," ").trim();return s?s.split(/\\s+/).length:0;}'
                                .'function rf(){var n=cW(ta.value);ctr.textContent=n+" / 1.000 palabras";ctr.style.color=n>LIMIT?"#c00":n>=900?"#d06000":"#555";ctr.style.fontWeight=n>=900?"bold":"";}'
                                .'var prev=ta.value;'
                                .'function sv(){if(cW(ta.value)<=LIMIT)prev=ta.value;}'
                                .'ta.addEventListener("keydown",sv);'
                                .'ta.addEventListener("paste",sv);'
                                .'ta.addEventListener("input",function(){if(cW(ta.value)>LIMIT){var p=ta.selectionStart;ta.value=prev;ta.selectionStart=ta.selectionEnd=Math.min(p,prev.length);}else{prev=ta.value;}rf();});'
                                .'rf();'
                                .'})();</script>';
                }
        }
