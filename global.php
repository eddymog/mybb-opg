...
if ($current_page == 'forumdisplay.php') {
	$g_forum_fid = $mybb->get_input('fid');
	$g_forum_oculto = '0'; 
	$g_forum_subisla = '0';
	$g_forum_parent_isla = '0';
	$g_forum_isla_rol = '0';

	$g_forum_fid_query = $db->query(" SELECT * FROM `mybb_forums` WHERE mybb_forums.fid = '$g_forum_fid'; ");
	while ($q = $db->fetch_array($g_forum_fid_query)) { $g_forum_oculto = $q['oculto']; $g_forum_subisla = $q['subisla']; $g_forum_parent_isla = $q['parent_isla']; $g_forum_isla_rol = $q['isla_rol']; }
...
$g_ultimos_post_query = $db->query(" SELECT th.tid, th.fid, th.subject, th.prefix, th.username, th.lastpost, th.lastposter, th.lastposteruid, th.uid, mybb_forums.name, mybb_usergroups.namestyle as namestyle FROM `mybb_threads` AS th INNER JOIN mybb_forums ON th.fid=mybb_forums.fid INNER JOIN mybb_users ON th.lastposteruid=mybb_users.uid INNER JOIN mybb_usergroups ON mybb_users.usergroup=mybb_usergroups.gid WHERE mybb_forums.pid != 27 AND mybb_forums.fid != 27 AND mybb_forums.parentlist NOT LIKE '%27,%' AND th.visible > 0 ORDER BY th.lastpost DESC LIMIT 20; ");
$g_ultimos_post_query_rol = $db->query(" SELECT th.tid, th.fid, th.subject, th.prefix, th.username, th.lastpost, th.lastposter, th.lastposteruid, th.uid, mybb_forums.name, mybb_usergroups.namestyle as namestyle FROM `mybb_threads` AS th INNER JOIN mybb_forums ON th.fid=mybb_forums.fid INNER JOIN mybb_users ON th.lastposteruid=mybb_users.uid INNER JOIN mybb_usergroups ON mybb_users.usergroup=mybb_usergroups.gid WHERE mybb_forums.pid != 27 AND mybb_forums.fid != 27 AND mybb_forums.fid = 10 OR mybb_forums.parentlist LIKE '%10,%' AND th.visible > 0 ORDER BY th.lastpost DESC LIMIT 20; ");
$g_ultimos_post_query_off = $db->query(" SELECT th.tid, th.fid, th.subject, th.prefix, th.username, th.lastpost, th.lastposter, th.lastposteruid, th.uid, mybb_forums.name, mybb_usergroups.namestyle as namestyle FROM `mybb_threads` AS th INNER JOIN mybb_forums ON th.fid=mybb_forums.fid INNER JOIN mybb_users ON th.lastposteruid=mybb_users.uid INNER JOIN mybb_usergroups ON mybb_users.usergroup=mybb_usergroups.gid WHERE mybb_forums.pid != 27 AND mybb_forums.fid != 27 AND mybb_forums.fid != 10 AND mybb_forums.parentlist NOT LIKE '%10,%' AND mybb_forums.parentlist NOT LIKE '%27,%' AND th.visible > 0 ORDER BY th.lastpost DESC LIMIT 20; ");
...
while ($q = $db->fetch_array($g_ultimos_post_query)) { 
	...
	$up_bg_color = $ultimos_post_count % 2 ? '#ffe3a0' : '#fcecd2';

	$g_ultimos_post .= "
		<div class='ultimos_post_section' style=' font-family: moonGetHeavy; font-size: 9px; color: black; padding: 7px 24px; background-color: $up_bg_color; '>
			<a href='$up_thread_url'><span style='color: #ff7e00;' class='username_bright'>$up_titulo</span></a>
			$up_fecha publicado a las $up_hora por <a href='$up_user_profile'><span>$up_username_display</span></a> En... 
			<a href='$up_forum'><span style='color: #ff7e00;' class='username_bright'>$up_forum_name</span></a>.
		</div>";
}
...
while ($q = $db->fetch_array($g_ultimos_post_query_rol)) { 
	...
	$g_ultimos_post_rol .= "
		<div class='ultimos_post_section' style=' font-family: moonGetHeavy; font-size: 9px; color: black; padding: 7px 24px; background-color: $up_bg_color; '>
			<a href='$up_thread_url'><span style='color: #ff7e00;'>$up_titulo</span></a>
			$up_fecha publicado a las $up_hora por <a href='$up_user_profile'><span>$up_username_display</span></a> En... 
			<a href='$up_forum'><span style='color: #ff7e00;'>$up_forum_name</span></a>.
		</div>";
}
...
// InicializaciÃ³n de variables condicionales de seguimiento (evita warnings en templates)
$g_crafteo_completo          = false;
$g_crafteo_en_curso          = false;
$g_creaciones_completo       = false;
$g_creaciones_en_curso       = false;
$g_entrenamiento_completo    = false;
$g_entrenamiento_en_curso    = false;
$g_entrenamiento_tecnica_completo = false;
$g_entrenamiento_tecnica_en_curso = false;
$g_oficio_completo           = false;
$g_oficio_en_curso           = false;
$g_recompensa_completa       = false;
$g_recompensa_en_curso       = false;
$g_recompensa_pendiente      = false;
$g_uiid                      = $mybb->user['uid']; // alias por typo en template ($g_uiid â†’ $g_uid)

$banner_number = rand(1, 56);
$banner_hentai_number = rand(1, 12);
...
if ($g_uid != 0) {
	$query_hentai = $db->query("
		SELECT * FROM mybb_op_hentai WHERE uid='$g_uid'
	");

	while ($q = $db->fetch_array($query_hentai)) {
		$g_enable_hentai = $q['enable_hentai'];
	}

	$g_ficha_query = $db->query(" SELECT * FROM mybb_op_fichas WHERE fid='$g_uid' ");

	while ($q = $db->fetch_array($g_ficha_query)) { $g_tiene_ficha = true; $g_ficha = $q; }

	if ($g_tiene_ficha) {
			// Revisar entrenamientos y misiones en curso
		$g_query_user_crafteo = $db->query("
			SELECT * FROM mybb_op_crafteo_usuarios WHERE uid='$g_uid'
		");

		while ($e = $db->fetch_array($g_query_user_crafteo)) {
			$efecto = 1;
			$time_now = time();
			
			$crafteo_duracion = intval($e['duracion']);
			$extra_time = $crafteo_duracion - ($crafteo_duracion * (1 / $efecto)); 

			$tiempo_crafteo = intval($e['timestamp_end'] - $extra_time) * 1000;
			$tid = $e['tid'] ?? null;

			if (time() > (intval($e['timestamp_end']) - $extra_time)) {
				$g_crafteo_completo = true;
			} else {
				$g_crafteo_en_curso = true;
			}
		}

		// ---

		$g_query_user_creaciones = $db->query("
			SELECT * FROM mybb_op_creacion_usuarios WHERE uid='$g_uid'
		");

		while ($e = $db->fetch_array($g_query_user_creaciones)) {
			$efecto = 1;
			$time_now = time();
			
			$creaciones_duracion = intval($e['duracion']);
			$extra_time = $creaciones_duracion - ($creaciones_duracion * (1 / $efecto)); 

			$tiempo_creaciones = intval($e['timestamp_end'] - $extra_time) * 1000;
			$tid = $e['tid'] ?? null;

			if (time() > (intval($e['timestamp_end']) - $extra_time)) {
				$g_creaciones_completo = true;
			} else {
				$g_creaciones_en_curso = true;
			}
		}

		$g_query_user_entrenamientos = $db->query("
			SELECT * FROM mybb_op_entrenamientos_usuarios WHERE uid='$g_uid'
		");

		while ($e = $db->fetch_array($g_query_user_entrenamientos)) {
			$efecto = 1;
			$time_now = time();
			...
			$tiempo_entreno = intval($e['timestamp_end'] - $extra_time) * 1000;
			$tid = $e['tid'] ?? null;

			if (time() > (intval($e['timestamp_end']) - $extra_time)) {
				$g_entrenamiento_completo = true;
			} else {
				$g_entrenamiento_en_curso = true;
			}
		}

		$g_query_user_oficios = $db->query("
			SELECT * FROM mybb_op_oficios_usuarios WHERE uid='$g_uid'
		");

		while ($e = $db->fetch_array($g_query_user_oficios)) {
			$efecto = 1;
			$time_now = time();

			$oficio_duracion = intval($e['duracion']);
			$extra_time = $oficio_duracion - ($oficio_duracion * (1 / $efecto)); 

			$tiempo_oficio = intval($e['timestamp_end'] - $extra_time) * 1000;
			$tid = $e['tid'] ?? null;

			if (time() > (intval($e['timestamp_end']) - $extra_time)) {
				$g_oficio_completo = true;
			} else {
				$g_oficio_en_curso = true;
			}
		}

		$g_query_user_tecnicas = $db->query("
			SELECT * FROM mybb_op_tecnicas_usuarios WHERE uid='$g_uid'
		");

		while ($e = $db->fetch_array($g_query_user_tecnicas)) {
			$efecto = 1;
			$time_now = time();
			
			$tecnica_duracion = intval($e['duracion']);
			$extra_time = $tecnica_duracion - ($tecnica_duracion * (1 / $efecto)); 

			$tiempo_tecnicas = intval($e['tiempo_finaliza'] - $extra_time) * 1000;
			$tid = $e['tid'] ?? null;

			if (time() > (intval($e['tiempo_finaliza']) - $extra_time)) {
				$g_entrenamiento_tecnica_completo = true;
			} else {
				$g_entrenamiento_tecnica_en_curso = true;
			}
		}

		// Revisar recompensa completada
		$g_two_days = 2 * 24 * 3600;
		$g_last_two_days = time() - $g_two_days;
		$g_should_accept = true;
		$g_days_count = 0;
		$g_time_to_accept = 0;
		$g_query_recompensa_actual = $db->query("
			SELECT * FROM mybb_op_recompensas_usuarios WHERE uid='$g_uid'
		");

		while ($q = $db->fetch_array($g_query_recompensa_actual)) {
			$g_days_count = $q['dia']; 
			$g_last_two_days = $q['tiempo'] - $g_two_days;
			$g_should_accept = time() > $q['tiempo'];
			$g_time_to_accept = $q['tiempo'] - time();
		}

		$g_query_posts = $db->query("
			...
			WHERE p.dateline > $g_last_two_days
		...
		$tiempo_recompensas = 0;
		$g_posts_counter = 0;

		while ($q = $db->fetch_array($g_query_posts)) {
			$g_posts_counter = $g_posts_counter + 1;
		}

		if ($g_posts_counter < 1 && $g_days_count > 0 && !$g_should_accept) {
			// time to write the two posts before it can be claimed again or it expires
			$tiempo_recompensas = $g_time_to_accept;
...
if (is_staff($g_uid) || is_mod($g_uid) || is_user($g_uid)) {
	$g_is_staff = true;
	$g_query_peticiones = $db->query("
		SELECT count(*) as numero_de_peticiones FROM `mybb_op_peticiones` WHERE resuelto=0 AND categoria != 'programacion'
	");
	$g_query_peticiones_bug = $db->query("
		SELECT count(*) as numero_de_peticiones FROM `mybb_op_peticiones` WHERE resuelto=0 AND categoria = 'programacion'
	");
	$g_query_peticiones_avisos = $db->query("
		SELECT count(*) as numero_de_peticiones FROM `mybb_op_avisos` WHERE resuelto=0
	");
	$g_query_fichas_en_cola = $db->query("
		SELECT count(*) as fichas_en_cola FROM `mybb_op_fichas` WHERE aprobada_por = 'sin_aprobar'
	");

	$query_total_creaciones_sin_moderar = $db->query("
		SELECT COUNT(*) as total_creaciones FROM mybb_threads as t
		INNER JOIN mybb_forums as f ON t.fid = f.fid 
	...
	$query_total_creaciones_pendientes = $db->query("
		SELECT COUNT(*) as total_creaciones FROM mybb_threads as t
		INNER JOIN mybb_forums as f ON t.fid = f.fid 
		AND f.fid = 8 AND t.closed = 0
		AND t.uid = t.lastposteruid
	...
	$g_peticiones = 0;
	$g_peticiones_bug = 0;
	$g_peticiones_avisos = 0;
	$g_fichas_en_cola = 0;
	$g_total_creaciones_sin_moderar = 0;
	$g_total_creaciones_pendientes = 0;

	while ($q = $db->fetch_array($g_query_peticiones)) {
		$g_peticiones = $q['numero_de_peticiones'];
	}

	while ($q = $db->fetch_array($g_query_peticiones_bug)) {
		$g_peticiones_bug = $q['numero_de_peticiones'];
	}

	while ($q = $db->fetch_array($g_query_peticiones_avisos)) {
		$g_peticiones_avisos = $q['numero_de_peticiones'];
	}

	while ($q = $db->fetch_array($g_query_fichas_en_cola)) {
		$g_fichas_en_cola = $q['fichas_en_cola'];
	}

	while ($q = $db->fetch_array($query_total_creaciones_sin_moderar)) {
		$g_total_creaciones_sin_moderar = $q['total_creaciones'];
	}

	while ($q = $db->fetch_array($query_total_creaciones_pendientes)) {
		$g_total_creaciones_pendientes = $q['total_creaciones'];
	}
}
...
// Safety init: variables de plugins que pueden no estar seteadas (accountswitcher, etc.)
if (!isset($as_header_dropdown)) $as_header_dropdown = '';
if (!isset($pm_switch_notice))   $pm_switch_notice   = '';
