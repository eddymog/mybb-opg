...
(function () {
  ...
  /* ── Viaje solitario ─────────────────────────────────────────── */
  // Islas de partida: incluye islas retiradas para permitir salir si el personaje ya está allí.
  var _SOLO_ISLAS = {
  ...
  // Islas de llegada: no incluye islas retiradas, ya no son destinos válidos.
  var _SOLO_ISLAS_DESTINO = {
    'East Blue':  ['Conomi Islands','Isla de Dawn','Refugio de Goat','Islas Organ','Tequila Wolf','Loguetown','Sabana de Cozia'],
  ...
  function _soloRebuildIslas(mar, selP, selL, valP, valL) {
    var islasPartida = _SOLO_ISLAS[mar] || [];
    var islasLlegada = _SOLO_ISLAS_DESTINO[mar] || [];
    [selP, selL].forEach(function (sel, idx) {
      var other = idx === 0 ? valL : valP;
      var islas = idx === 0 ? islasPartida : islasLlegada;
  ...
  window.baiViajeMarChanged = function () {
    ...
    var islasPartida = _MARES_ISLAS[mar.value] || [];
    var islasLlegada = _MARES_ISLAS_DESTINO[mar.value] || [];
    var optsPartida = islasPartida.map(function (i) { return '<option value="' + esc(i) + '">' + esc(i) + '</option>'; }).join('');
    var optsLlegada = islasLlegada.map(function (i) { return '<option value="' + esc(i) + '">' + esc(i) + '</option>'; }).join('');
    var empty = '<option value="">— Selecciona —</option>';
    partida.innerHTML = empty + optsPartida;
    llegada.innerHTML = empty + optsLlegada;
  };
