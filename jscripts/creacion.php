...
if ($ficha_existe == true && $ficha_aprobada == true) {
    ...
    foreach ($creacion_costos_nuevos as $t_id => $t_info) {
        ...
        $g_creacion_botones .= '<div class="item-outer creacion-card">
            <div class="item-nombre">' . $nombre_safe . '</div>
            <div class="creacion-costo">' . $costo_safe . '</div>
            <button type="button" class="mini-button crear-button" onclick="confirmarCreacion(\'' . $t_id . '\', \'' . $nombre_safe . '\', \'' . $costo_safe . '\')">Crear</button>
        </div>';
    }
