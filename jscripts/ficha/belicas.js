...
(function (FICHA, w) {
  ...
  // ── Rejilla de disciplinas (generada desde el catálogo de BD) ──
  //
  // Antes eran 12 bloques <img>/<div> fijos en op_ficha_belico.html (uno por
  // disciplina), con sus ids hardcodeados. Ahora la plantilla solo trae un
  // contenedor vacío (#op_disciplinas_grid) y esta función construye un
  // bloque por cada entrada de window.OPG_CATALOGO_DISCIPLINAS (nombre,
  // camino1, camino2, imagen) — así una disciplina añadida desde
  // OPG Catálogos aparece sin tocar plantilla ni JS.

  function buildCaminoPill(caminoNombre) {
    var caminoId = normBelicaId(caminoNombre);
    // Nombres largos (ej. "WeaponMaster") no caben a 10px en la píldora de
    // 100px — mismo ajuste que ya existía a mano para ese caso concreto.
    var fontSize = caminoNombre.length > 10 ? '8px' : '10px';
    var pill = document.createElement('div');
    pill.style.cssText = 'background-color: rgba(0,0,0,0.7); width: 100px; height: 18px; border-radius: 10px; display: flex; align-items: center;';
    pill.innerHTML =
      '<div id="' + caminoId + '_nivel_body" style="background-color: white; height: 12px; width: 12px; border-radius: 10px; margin-left: 3px; display: flex; align-items: center; justify-content: center;">' +
        '<span id="' + caminoId + '_nivel" style="font-family: \'moonGetHeavy\'; font-size: 8px; color: black;"></span>' +
      '</div>' +
      '<span style="font-family: \'moonGetHeavy\'; color: white; font-size: ' + fontSize + '; margin-left: 5px; text-shadow: 1px 1px 0px black;">' + caminoNombre + '</span>';
    return pill;
  }

  function buildDisciplinaBlock(d) {
    var nombreId = normBelicaId(d.nombre);
    var box = document.createElement('div');
    box.style.cssText = 'width: 215px; height: 95px; margin: 0 5px; position: relative; border-radius: 5px; overflow: hidden; border: 2px solid #000;';

    var img = document.createElement('img');
    img.id = nombreId + '_img';
    img.src = d.imagen || '';
    img.style.cssText = 'width: 100%; height: 100%; object-fit: cover; position: absolute; filter: grayscale(1);';
    box.appendChild(img);

    var header = document.createElement('div');
    header.style.cssText = 'position: relative; height: 20px; background-color: rgba(0,0,0,0.7); display: flex; justify-content: space-between; align-items: center; padding: 0 5px;';
    header.innerHTML =
      '<span style="font-family: \'moonGetHeavy\'; color: white; font-size: 12px; text-shadow: 1px 1px 0px black;">' + d.nombre.toUpperCase() + '</span>' +
      '<div id="' + nombreId + '_nivel_body" style="background-color: white; height: 14px; width: 14px; border-radius: 10px; display: flex; align-items: center; justify-content: center; cursor: pointer;">' +
        '<span id="' + nombreId + '_nivel" style="font-family: \'moonGetHeavy\'; font-size: 10px; color: black;"></span>' +
      '</div>';
    box.appendChild(header);

    var subArea = document.createElement('div');
    subArea.style.cssText = 'position: absolute; bottom: 5px; left: 0; right: 0; display: flex; justify-content: center; gap: 5px;';
    subArea.appendChild(buildCaminoPill(d.camino1));
    subArea.appendChild(buildCaminoPill(d.camino2));
    box.appendChild(subArea);

    return box;
  }

  function renderDisciplinasGrid(catalogo) {
    var container = document.getElementById('op_disciplinas_grid');
    if (!container || !catalogo || !catalogo.length) return;
    container.innerHTML = '';

    var POR_FILA = 4;
    for (var i = 0; i < catalogo.length; i += POR_FILA) {
      var fila = document.createElement('div');
      fila.style.cssText = 'display: flex; justify-content: center; margin-bottom: 10px;';
      catalogo.slice(i, i + POR_FILA).forEach(function (d) {
        fila.appendChild(buildDisciplinaBlock(d));
      });
  ...
  w.renderDisciplinasGrid       = renderDisciplinasGrid;
