# Herramienta de staff para los banners: plan de implementación

> Diseño: [banners-staff-design.md](banners-staff-design.md)

**Objetivo:** crear `op/staff/banners.php` para que el staff pueda subir, desactivar, eliminar, restaurar y forzar los banners de la rotación del header, y enlazarla desde la consola.

**Requisitos previos:**
- El plugin `op_banner_rotativo` está en `inc/plugins/` (no hace falta que esté activo para que la página funcione, pero sí para que los banners roten).
- El servidor tiene GD (`php -m | grep gd`).

**Archivos:**

| Acción | Archivo |
|---|---|
| Crear | `op/staff/banners.php` |
| Modificar | `inc/plugins/op_banner_rotativo.php` (añadir `?v=` a la URL) |
| Modificar | `templates/One_Piece_Gaiden_Templates/staff_consola_mod.html` (botón nuevo) |

No hay migraciones SQL. Las carpetas `_inactivos/` y `_papelera/` se crean solas la primera vez que se usan.

---

## Tarea 1: poner versión a la URL del banner en el plugin

**Archivo:** `inc/plugins/op_banner_rotativo.php`, última línea de `op_banner_rotativo_run()`.
...
**Verificación:** `php -l inc/plugins/op_banner_rotativo.php`, y comprobar que en el foro el `<img>` de `#logo` termina en `?v=<número>`.

## Tarea 2: crear la página de staff

**Archivo:** `op/staff/banners.php`
...
**Notas:**
- `banners_guardar_subida()` devuelve `''` si todo ha ido bien o el texto del error. En el `switch`, `$ok` solo se usa cuando `$error === ''`.
- `banners_mover()` crea la carpeta de destino si no existe (`0755`).
- `$banners_protegidos` (59 y 60) no se pueden desactivar ni eliminar, porque `header.html` los usa con ruta fija. Sí se pueden fijar. Si la plantilla cambia, hay que actualizar esa lista.
- Tampoco se puede desactivar ni eliminar el banner fijado: primero hay que quitarlo del panel *Banner fijo*.
- `MyBB::INPUT_INT` convierte `n` a entero. Aunque alguien envíe `n=../../x`, se queda en `0`, y `banners_estado(0)` devuelve `null`.
- El aviso tras un POST ("Banner N fijado", errores, etc.) viaja en una cookie de un solo uso (`BANNERS_MSG_COOKIE`), no en la URL: así el redirect va a `banners.php` sin query string y "Actualizar" no repite la acción ni deja el mensaje pegado en la barra de direcciones. La cookie se codifica en base64 porque `my_setcookie()` aplica `urlencode()` y PHP no deshace el "+" de los espacios al leer `$_COOKIE` (sí decodifica `%XX`), así que un mensaje con espacios volvería con "+" literales sin ese paso. Se lee y se borra en la misma petición (`my_unsetcookie()`), así que sobrevive exactamente a un refresco.

**Verificación:**
1. `php -l op/staff/banners.php` → `No syntax errors detected`.

## Tarea 3: botón en la consola

**Archivo:** `templates/One_Piece_Gaiden_Templates/staff_consola_mod.html`, categoría *Utilidades*.

```html
                <a class="boton" href="/op/staff/sabiasque_modificar.php">Modificar Sabías Qué</a>
                <a class="boton" href="/op/staff/banners.php">Banners del header</a>
```

`template_sync` pasa el cambio a la base de datos en unos 10 segundos después de subirlo por FTP.

## Tarea 4: probar en el servidor

Todo con un usuario de staff salvo que se indique lo contrario:

1. **Acceso:** abrir `/op/staff/banners.php` con un usuario que no sea staff (o como invitado) → aparece la página de "No tienes acceso". Con staff → cuadrícula con los 61 banners activos, y el actual marcado como "En el header ahora".
2. **CSRF:** enviar un POST sin `my_post_key` (por ejemplo, con las DevTools) → MyBB lo rechaza con su error de verificación.
3. **Subir:**
   - Un PNG de 2200×1240 → "Banner 62 subido…". En el servidor queda `Banner62_One_Piece_Gaiden_Foro_Rol.jpg` de 1100×620.
   - Una imagen de 500×500 → error de proporción.
   - Un `.jpg` que en realidad es texto → "no es una imagen… válida".
   - Pulsar F5 después de subir → no se crea un Banner 63 (gracias al Post/Redirect/Get).
4. **Banner fijo:** escribir 62 en "Banner nº" y pulsar *Fijar*. El índice del foro muestra el 62 aunque pasen más de 5 minutos, y la tarjeta del 62 dice "Fijado", sin *Desactivar* ni *Eliminar*. *Quitar* hace que el header vuelva a rotar. Fijar un número que no está activo da error.
5. **Desactivar** el 62 (sin fijar) mientras se está mostrando → en la siguiente carga el header muestra otro banner, nunca una imagen rota. El 62 aparece en *Inactivos*.
6. **Protegidos:** el 59 y el 60 llevan la etiqueta "Protegido" y no tienen botones de *Desactivar* ni *Eliminar*. Si se fuerza un POST `accion=eliminar&n=60`, sale un error y el archivo no se mueve.
7. **Eliminar** desde *Inactivos* → pasa a *Papelera*. **Restaurar** → vuelve a *Activos*.
8. **Numeración:** con el 62 en la papelera, subir otro → recibe el número 63, no el 62.
9. **Auditoría:**
   ```sql
   SELECT * FROM mybb_op_audit_general WHERE categoria = 'banners' ORDER BY id DESC LIMIT 10;
   ```
   Aparece una fila por cada acción que salió bien, con el usuario correcto.

Al terminar, eliminar o restaurar los banners de prueba según convenga.

## Tarea 5: commit

```bash
git add op/staff/banners.php inc/plugins/op_banner_rotativo.php templates/One_Piece_Gaiden_Templates/staff_consola_mod.html
git commit -m "Herramienta de staff para gestionar los banners del header"
```
...
- Borrar `op/staff/banners.php` y quitar el botón de la consola. Los banners que estén en `_inactivos/` o `_papelera/` se quedan donde están. Para recuperarlos, se mueven a la raíz por FTP.
- El cambio del `?v=` en el plugin es independiente y se puede dejar puesto.
