...
if ($ficha && in_array($ficha['aprobada_por'], array('sin_aprobar', 'pendiente_reset'), true)) {
    $mensaje_redireccion = "Tu ficha no está aprobada. Usa tu Ticket de Reset de Build si está pendiente de reset.";
    eval("\$page = \"".$templates->get("op_redireccion")."\";");
    output_page($page);
    return;
}
...
if ($accion == 'kenbun') {
    ...
    if ($kenbun == 6 && ($nivel >= 50 || ($has_full_haki && $nivel >= 45))) { $nikasCosto = 150; }
    if ($kenbun == 7 && ($nivel >= 666 || ($has_full_haki && $nivel >= 60))) { $nikasCosto = 8; }

    if ($nikas >= $nikasCosto && $kenbunNuevo >= 2 && $kenbunNuevo <= 8) {
        $nikasNuevo = $nikas - $nikasCosto;
...
if ($accion == 'buso') {
    ...
    if ($buso == 6 && ($nivel >= 50 || ($has_full_haki && $nivel >= 45))) { $nikasCosto = 150; }
    if ($buso == 7 && ($nivel >= 666 || ($has_full_haki && $nivel >= 60))) { $nikasCosto = 8; }

    if ($nikas >= $nikasCosto && $busoNuevo >= 2 && $busoNuevo <= 8) {
        $nikasNuevo = $nikas - $nikasCosto;
...
if ($accion == 'hao') {
    ...
    if ($hao == 6 && ($nivel >= 50 || ($has_full_haki && $nivel >= 45))) { $nikasCosto = 150; }
    if ($hao == 7 && ($nivel >= 666 || ($has_full_haki && $nivel >= 60))) { $nikasCosto = 8; }

    if ($nikas >= $nikasCosto && $haoNuevo >= 2 && $haoNuevo <= 8) {
        $nikasNuevo = $nikas - $nikasCosto;
...
if ($accion == 'estilo2_desbloquear') {
    $nikasNuevo = $nikas - 50;
    if ($nikasNuevo >= 0 && $nivel >= 20 && $ficha['estilo2'] == 'bloqueado') {
        $db->query(" UPDATE `mybb_op_fichas` SET `estilo2`='no_bloqueado' WHERE `fid`='$user_uid'; ");
        log_audit($user_uid, $username, $user_uid, '[Mejoras][Estilo 2]', "$nikas->$nikasNuevo (Gasto: 50).");
...
if ($accion == 'estilo3_desbloquear') {
    $nikasNuevo = $nikas - 100;
    if ($nikasNuevo >= 0 && $nivel >= 35 && $ficha['estilo3'] == 'bloqueado') {
        $db->query(" UPDATE `mybb_op_fichas` SET `estilo3`='no_bloqueado' WHERE `fid`='$user_uid'; ");
        log_audit($user_uid, $username, $user_uid, '[Mejoras][Estilo 3]', "$nikas->$nikasNuevo (Gasto: 100).");
...
// estilo4_desbloquear ya no existe: el cuarto slot de estilo se retiró.
...
if ($accion == 'oficio') {
    ...
    if ($oficioNumber === 'oficio2' && !$has_polivalente && !$has_erudito) {
        echo("<script>alert('Necesitas la virtud Polivalente o Erudito para tener un segundo oficio.');window.location.href = 'https://onepiecegaiden.com/op/personaje.php?uid=$user_uid';</script>");
        return;
    }
    $oficios = json_decode($ficha['oficios']);
    $textoSubir = "";
...
if ($accion == 'belica') {
    ...
    // actual (no confiar en $belicaNumber) para que no se pueda saltar manipulando la URL.
    $disciplinas_desbloqueadas = 0;
    for ($bi = 1; $bi <= 12; $bi++) {
        if (!empty($ficha["belica{$bi}"])) { $disciplinas_desbloqueadas++; }
    }
    if ($disciplinas_desbloqueadas >= 6) {
        echo("<script>alert('Ya tienes el máximo de 6 disciplinas desbloqueadas.');window.location.href = 'https://onepiecegaiden.com/op/personaje.php?uid=$user_uid';</script>");
        return;
    }
...
if ($accion == 'oficio_espe') {  
    ...
    if (isset($oficios->{$oficio}->{$espeNumber})) { 
        ...
        $mejoras_oficio_ya_tiene_espe = isset($oficios->{$oficio}->{'espe1'}) || isset($oficios->{$oficio}->{'espe2'});
        if ($mejoras_oficio_ya_tiene_espe) {
            $mejoras_max_espe_2 = ($oficioNumber === 'oficio1') ? ($has_estudioso || $has_erudito) : $has_erudito;
            if (!$mejoras_max_espe_2) {
                echo("<script>alert('No puedes tener una segunda especialización en este oficio sin la virtud correspondiente.');window.location.href = 'https://onepiecegaiden.com/op/personaje.php?uid=$user_uid';</script>");
                return;
            }
        }
