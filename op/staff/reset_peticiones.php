...
function reset_collect_propuesta($backup, $belica_valid, $oficio_valid, $camino_valid, $original_propuesta = [], $target_uid = 0) {
    global $caminos_map_r, $db;
    $p = [];

    // Virtudes que afectan a oficios: Estudioso (V035), Polivalente (V036), Erudito (V028).
    $rp_estudioso = false; $rp_polivalente = false; $rp_erudito = false;
    if ($target_uid > 0) {
        $rp_virt_q = $db->query("SELECT virtud_id FROM mybb_op_virtudes_usuarios WHERE uid='".(int)$target_uid."' AND virtud_id IN ('V035','V036','V028')");
        while ($rpvq = $db->fetch_array($rp_virt_q)) {
            if ($rpvq['virtud_id'] === 'V035') $rp_estudioso = true;
            if ($rpvq['virtud_id'] === 'V036') $rp_polivalente = true;
            if ($rpvq['virtud_id'] === 'V028') $rp_erudito = true;
        }
    }
    // tecs_perder/tecs_ganar no tienen campos propios en el formulario de staff (no se
    // re-recogen de POST): se preservan tal cual venían de la propuesta original del
    // jugador, para no perderlos cada vez que se guarda/aplica.
    $p['tecs_perder'] = $original_propuesta['tecs_perder'] ?? [];
    $p['tecs_ganar']  = $original_propuesta['tecs_ganar']  ?? [];
    $p['resetear_atributos'] = !empty($_POST['p_resetear_atributos']);
    // Sólo 6 slots son elegibles en el reset (igual que el 4º estilo retirado); 7-12 se
    // fuerzan siempre a vacío, sin importar lo que llegue por POST, para que se limpien
    // y reembolsen automáticamente si el backup todavía los tiene ocupados.
    for ($i = 1; $i <= 12; $i++) {
        if ($i > 6) { $p["belica$i"] = ''; continue; }
    ...
    // El 4º slot de estilo se retiró: el ticket de reset (TNP001) sólo permite elegir 3.
    for ($i = 1; $i <= 3; $i++) {
    ...
    // El segundo oficio requiere Polivalente o Erudito; si no, se fuerza vacío.
    if ($p['oficio2'] !== '' && !$rp_polivalente && !$rp_erudito) {
        $p['oficio2'] = '';
    }
    ...
    for ($oi = 1; $oi <= 2; $oi++) {
        ...
        // Estudioso o Erudito. Oficio2: 2 requiere Erudito. Si no se cumple, se
        // fuerza la segunda especialización a 0 en vez de rechazar (staff conserva
        // control total sobre el resto de la propuesta).
        if ($p["oficio{$oi}_sub1_nivel"] >= 1 && $p["oficio{$oi}_sub2_nivel"] >= 1) {
            $rp_max_espe_2 = ($oi === 1) ? ($rp_estudioso || $rp_erudito) : $rp_erudito;
            if (!$rp_max_espe_2) {
                $p["oficio{$oi}_sub2_nivel"] = 0;
            }
        }
    }
    // Los tres pueden ser -1 (no despertado) o 0 (despertado, nivel base). Forzar un
    // suelo de 1 rompía cualquier propuesta que partiera de esos valores (el input ni
    // siquiera dejaba enviarla).
    $p['kenbun']        = max(-1, min(8, (int)($_POST['p_kenbun'] ?? 1)));
    $p['buso']          = max(-1, min(8, (int)($_POST['p_buso']   ?? 1)));
    $p['hao']           = max(-1, min(8, (int)($_POST['p_hao']    ?? 1)));
...
if ($accion === 'rechazar' && $reset_id > 0) {
    ...
    if ($reset) {
        ...
        log_audit($uid, $username, '[Reset Build]', "Reset #$reset_id (ficha #$target_uid) rechazado. Ticket devuelto.");
...
if ($accion === 'guardar' && $reset_id > 0) {
    $rq = $db->query("SELECT uid, backup_json, propuesta_json FROM mybb_op_fichas_reset WHERE id='$reset_id' AND estado='pendiente' LIMIT 1");
    $rr = $db->fetch_array($rq);
    if ($rr) {
        $backup     = json_decode($rr['backup_json'], true) ?: [];
        $orig_prop  = json_decode($rr['propuesta_json'], true) ?: [];
        $propuesta = reset_collect_propuesta($backup, $belica_valid, $oficio_valid, $camino_valid, $orig_prop, (int)$rr['uid']);
...
if ($accion === 'aplicar' && $reset_id > 0) {
    ...
    if (!$reset) {
        ...
        $orig_prop  = json_decode($reset['propuesta_json'], true) ?: [];

        // Update propuesta from POST before applying
        $propuesta  = reset_collect_propuesta($backup, $belica_valid, $oficio_valid, $camino_valid, $orig_prop, $target_uid);
        $prop_esc   = $db->escape_string(json_encode($propuesta, JSON_UNESCAPED_UNICODE));
        $db->query("UPDATE mybb_op_fichas_reset SET propuesta_json='$prop_esc' WHERE id='$reset_id'");
        ...
        if (!$ficha) {
            ...
            if ($belicas_changed) {
                // json_encode([]) da '[]' (array JSON), no '{}' (objeto JSON) — el resto del
                // código (personaje.php, etc.) espera siempre un objeto al hacer json_decode()
                // y acceder con ->, así que un array vacío rompería esa lectura más adelante.
                $belicas_json = empty($cur_belicas) ? '{}' : json_encode($cur_belicas, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                $set_clauses[] = "belicas='".$db->escape_string($belicas_json)."'";
            }
            ...
            if ($oficios_changed) {
                // Mismo motivo que con belicas más arriba: forzar '{}' en vez de '[]' cuando
                // queda vacío, para no romper el json_decode()+"->" que se hace en otros sitios.
                $oficios_json = empty($cur_oficios) ? '{}' : json_encode($cur_oficios, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                $set_clauses[] = "oficios='".$db->escape_string($oficios_json)."'";
            }
            ...
            $slot_costs_e_old = ['estilo2'=>25,'estilo3'=>50,'estilo4'=>75]; // precios antiguos: reembolso del build actual
            $slot_costs_e_new = ['estilo2'=>50,'estilo3'=>100]; // precios nuevos: coste del build propuesto
            ...
            // Coste acumulado de haki (kenbun/buso/hao) desde nivel 1, y de Dominio Akuma desde 0
            // (mismo criterio que las acciones directas 'kenbun'/'buso'/'hao'/'control_akuma').
            $rst_haki_cost = function($nivel_actual, $is_haki_camino) {
                if ($nivel_actual <= 1) return 0;
                $costs = [1=>10, 2=>15, 3=>25, 4=>40, 5=>60, 6=>150, 7=>8];
                if ($is_haki_camino) $costs[1] = 0;
                $total = 0;
                for ($cur = 1; $cur < $nivel_actual; $cur++) { $total += $costs[$cur] ?? 0; }
                return $total;
            };
            $rst_akuma_cost = function($dom_actual, $is_akuma_camino) {
                if ($dom_actual <= 0) return 0;
                $costs = $is_akuma_camino ? [0=>0,1=>5,2=>15,3=>20,4=>30,5=>80] : [0=>5,1=>10,2=>20,3=>25,4=>40];
                $total = 0;
                for ($d = 0; $d < $dom_actual; $d++) { $total += $costs[$d] ?? 0; }
                return $total;
            };
            ...
            for ($i = 2; $i <= 4; $i++) {
                if (($backup["estilo$i"] ?? 'bloqueado') !== 'bloqueado') $old_build_cost += $slot_costs_e_old["estilo$i"];
            }
            $bak_is_haki  = (($backup['camino'] ?? '') === 'Haki');
            $bak_is_akuma = (($backup['camino'] ?? '') === 'Akuma');
            $old_build_cost += $rst_haki_cost(max(0, (int)($backup['kenbun'] ?? 0)), $bak_is_haki);
            $old_build_cost += $rst_haki_cost(max(0, (int)($backup['buso'] ?? 0)), $bak_is_haki);
            if ((int)($backup['hao'] ?? 0) > 0) $old_build_cost += $rst_haki_cost((int)$backup['hao'], $bak_is_haki);
            $old_build_cost += $rst_akuma_cost(max(0, (int)($backup['dominio_akuma'] ?? 0)), $bak_is_akuma);
            for ($i = 1; $i <= 12; $i++) {
            ...
            for ($i = 2; $i <= 3; $i++) {
                if (($propuesta["estilo$i"] ?? 'bloqueado') !== 'bloqueado') $new_build_cost += $slot_costs_e_new["estilo$i"];
            }
            for ($i = 1; $i <= 12; $i++) {
                $e1 = $propuesta["belica{$i}_espe1"] ?? '';
                if ($e1 !== '') {
            ...
            $prop_is_haki  = (($propuesta['camino'] ?? '') === 'Haki');
            $prop_is_akuma = (($propuesta['camino'] ?? '') === 'Akuma');
            $new_build_cost += $rst_haki_cost(max(0, (int)($propuesta['kenbun'] ?? 0)), $prop_is_haki);
            $new_build_cost += $rst_haki_cost(max(0, (int)($propuesta['buso'] ?? 0)), $prop_is_haki);
            if ((int)($propuesta['hao'] ?? 0) > 0) $new_build_cost += $rst_haki_cost((int)$propuesta['hao'], $prop_is_haki);
            $new_build_cost += $rst_akuma_cost(max(0, (int)($propuesta['dominio_akuma'] ?? 0)), $prop_is_akuma);
            ...
            // Levantar el bloqueo de pendiente_reset: la ficha vuelve a estar aprobada.
            if (($ficha['aprobada_por'] ?? '') === 'pendiente_reset') {
                $set_clauses[] = "aprobada_por='".$db->escape_string($username)."'";
                $log_parts[] = "aprobada_por:pendiente_reset→$username";
            }

            // Cualquier técnica de disciplina/estilo/camino cuya rama NO pertenezca a una
            // fuente del build PROPUESTO final se fuerza a "perder" (recalculado aquí por si
            // staff cambió el build tras el envío original). Se compara contra el build
            // propuesto directamente (no un diff backup-vs-propuesto) para atrapar también
            // técnicas ya huérfanas de ANTES de este reset — p.ej. de un 4º estilo o una
            // disciplina >6 ya limpiados por las migraciones de staff, cuyas técnicas seguían
            // aprendidas sin que nada las justificara.
            $estilo_valid_r = ['Gunkata','Hasshoken','Santoryu','Kuroashi','Gyojin Karate','Gyojin Bukijutsu','Gyojin Jujutsu','Rokushiki','Okama Kempo','Sora Yokujin','Ninjutsu','Ryusoken','Jiyuumura Kempo','Pop Green','Clima Tact','Funekiri','Hakai Shin','Railgun Style','Shuron Hakke','Raqisat Alsahra','Breeskjold','Impacto Explosivo','Royal Guard','Shikaku Teikoku','Duelliste de Givre','Filo Della Vita','Havets Symfoni','Bakudai Karin','Yama Kurai','Shiseiju','Sea Corsair','Wano Nitoryu','Mano de Tahur','Kokudan','Kodai no Bushido','Ittoryu Sekai','Global Performer','Cavalry Warrior','Ashigara Dokoi','Kanpo Kenpo'];
            $caminos_flat_r = [];
            foreach ($caminos_map_r as $disc_names_r) { foreach ($disc_names_r as $cn_r) { $caminos_flat_r[] = $cn_r; } }
            $all_valid_sources_r = array_merge($belica_valid, $estilo_valid_r, $caminos_flat_r);

            $prop_sources_r = [];
            for ($i = 1; $i <= 6; $i++) {
                $disc = $propuesta["belica$i"] ?? '';
                if ($disc === '') continue;
                $prop_sources_r[$disc] = true;
                $e1r = $propuesta["belica{$i}_espe1"] ?? ''; if ($e1r !== '') $prop_sources_r[$e1r] = true;
                $e2r = $propuesta["belica{$i}_espe2"] ?? ''; if ($e2r !== '') $prop_sources_r[$e2r] = true;
            }
            for ($i = 1; $i <= 3; $i++) {
                $est = $propuesta["estilo$i"] ?? '';
                if ($est !== '' && $est !== 'bloqueado' && $est !== 'no_bloqueado') $prop_sources_r[$est] = true;
            }

            $forced_perder_r = [];
            $q_all_learned_r = $db->query("SELECT t.tid, t.rama FROM mybb_op_tecnicas t INNER JOIN mybb_op_tec_aprendidas ta ON t.tid=ta.tid WHERE ta.uid='$target_uid'");
            while ($rlr = $db->fetch_array($q_all_learned_r)) {
                $rama_r = $rlr['rama'];
                $es_disciplina_o_estilo_r = false;
                foreach ($all_valid_sources_r as $vs_r) {
                    if (strpos($rama_r, $vs_r) !== false) { $es_disciplina_o_estilo_r = true; break; }
                }
                if (!$es_disciplina_o_estilo_r) continue; // técnica especial/única/racial/etc.: nunca se fuerza
                $matches_proposed_r = false;
                foreach ($prop_sources_r as $src_name_r => $_) {
                    if (strpos($rama_r, $src_name_r) !== false) { $matches_proposed_r = true; break; }
                }
                if (!$matches_proposed_r) { $forced_perder_r[] = $rlr['tid']; }
            }
            if (!empty($forced_perder_r)) {
                $propuesta['tecs_perder'] = array_values(array_unique(array_merge((array)($propuesta['tecs_perder'] ?? []), $forced_perder_r)));
            }

            // Ajuste de pasivas por técnicas ganadas/perdidas en el intercambio (mismo mapa que
            // op/staff/ficha_recalcular_pasivas.php y op/entrenamiento_tecnicas.php). Los
            // virtudes/defectos no se ven afectados: viven en mybb_op_virtudes_usuarios, tabla
            // aparte que este intercambio de técnicas no toca.
            $tec_pasiva_map_r = [
                ...
                'RAC001' => ['voluntad_pasiva' => 15],
                'TRI001' => ['fuerza_pasiva' => 5], 'TRI002' => ['agilidad_pasiva' => 5], 'TRI003' => ['punteria_pasiva' => 5],
                'TRI004' => ['destreza_pasiva' => 5], 'TRI005' => ['punteria_pasiva' => 5],
                'RAC006' => ['fuerza_pasiva' => 30, 'resistencia_pasiva' => 30, 'destreza_pasiva' => 10, 'agilidad_pasiva' => -10],
                'TRI007' => ['fuerza_pasiva' => 5],
                'RAC003' => ['reflejos_pasiva' => 5, 'agilidad_pasiva' => 15, 'fuerza_pasiva' => 10],
                'RAC008' => ['agilidad_pasiva' => 10, 'destreza_pasiva' => 10],
                'RAC002' => ['fuerza_pasiva' => 15],
                'RAC009' => ['punteria_pasiva' => 15],
                'RAC005' => ['fuerza_pasiva' => 15, 'resistencia_pasiva' => 15, 'voluntad_pasiva' => 5],
                'RAC004' => ['resistencia_pasiva' => 10, 'voluntad_pasiva' => 10, 'agilidad_pasiva' => 10],
                'RAC007' => ['reflejos_pasiva' => 10],
                'TRI010' => ['resistencia_pasiva' => 5],
                'TRI009' => ['agilidad_pasiva' => 5],
                'RAC010' => ['fuerza_pasiva' => 10, 'resistencia_pasiva' => 10, 'voluntad_pasiva' => 10],
                'RAC013' => ['fuerza_pasiva' => 10, 'resistencia_pasiva' => 10, 'voluntad_pasiva' => 5, 'agilidad_pasiva' => 5],
                'RAC018' => ['voluntad_pasiva' => 10],
                'RAC011' => ['fuerza_pasiva' => 10, 'resistencia_pasiva' => 10],
                'RAC016' => ['punteria_pasiva' => 15, 'agilidad_pasiva' => 15],
                'RAC017' => ['resistencia_pasiva' => -5],
                'RAC014' => ['agilidad_pasiva' => 10],
                'RAC015' => ['fuerza_pasiva' => 10, 'resistencia_pasiva' => 10],
                'RAC019' => ['agilidad_pasiva' => 10, 'destreza_pasiva' => 10],
                'RAC012' => ['fuerza_pasiva' => 5, 'voluntad_pasiva' => 10],
                'RAC020' => ['agilidad_pasiva' => 10, 'reflejos_pasiva' => 10, 'resistencia_pasiva' => 10],
                'RAC021' => ['voluntad_pasiva' => 10, 'fuerza_pasiva' => 10, 'resistencia_pasiva' => 10],
                'RAC022' => ['fuerza_pasiva' => 10, 'agilidad_pasiva' => 10],
            ];
            $pasiva_delta_r = [];
            foreach ((array)($propuesta['tecs_perder'] ?? []) as $tec_del_p) {
                if (isset($tec_pasiva_map_r[$tec_del_p])) {
                    foreach ($tec_pasiva_map_r[$tec_del_p] as $col_p => $val_p) {
                        $pasiva_delta_r[$col_p] = ($pasiva_delta_r[$col_p] ?? 0) - $val_p;
                    }
                }
            }
            foreach ((array)($propuesta['tecs_ganar'] ?? []) as $tec_add_p) {
                if (isset($tec_pasiva_map_r[$tec_add_p])) {
                    foreach ($tec_pasiva_map_r[$tec_add_p] as $col_p => $val_p) {
                        $pasiva_delta_r[$col_p] = ($pasiva_delta_r[$col_p] ?? 0) + $val_p;
                    }
                }
            }
            foreach ($pasiva_delta_r as $col_p => $delta_p) {
                if ($delta_p !== 0) {
                    $set_clauses[] = "$col_p=$col_p" . ($delta_p >= 0 ? "+$delta_p" : $delta_p);
                    $log_parts[] = "$col_p:" . ($delta_p >= 0 ? "+$delta_p" : $delta_p) . " (por técnicas)";
                }
            }
            ...
            log_audit($uid, $username, '[Reset Build]', "Reset #$reset_id (ficha #$target_uid) aplicado: $log_str");
...
if ($reset_id > 0) {
    ...
    if (!$reset) {
        ...
        // Belica selects / text display (with caminos). Sólo 6 slots son elegibles en el
        // reset (igual que el 4º estilo retirado); slots 7-12 se muestran aparte, sólo
        // informativos, más abajo.
        ...
        // Disciplinas en slots 7-12: no elegibles en el reset, sólo informativas. Si el backup
        // todavía tiene alguna (ficha con más de 6 disciplinas de antes de este límite), se
        // eliminará automáticamente (con reembolso) al aplicar — igual que el 4º estilo.
        for ($i = 7; $i <= 12; $i++) {
            $bak_extra = $backup["belica$i"] ?? '';
            if ($bak_extra === '') continue;
            $belica_rows .= '<div class="rst-slot"><div class="rst-slabel">Disciplina extra (retirada)</div>'
                .'<div>'.htmlspecialchars($bak_extra, ENT_QUOTES).' <span class="rst-bak">(se eliminará al aplicar el reset, con reembolso)</span></div></div>';
        }

        // Estilo inputs (el 4º slot se retiró: el ticket de reset sólo permite elegir 3)
        $estilo_rows = '';
        $estilo_labels = ['Estilo 1','Estilo 2 (50✦)','Estilo 3 (100✦)'];
        for ($i = 1; $i <= 3; $i++) {
            $cls = $diff("estilo$i");
        ...
        // El 4º estilo del backup (si lo hay) se muestra sólo informativamente: se limpiará al aplicar el reset.
        $bak_estilo4 = $backup['estilo4'] ?? '';
        if ($bak_estilo4 !== '' && $bak_estilo4 !== 'bloqueado' && $bak_estilo4 !== 'no_bloqueado') {
            $estilo_rows .= '<div class="rst-slot"><div class="rst-slabel">Estilo 4 (retirado)</div><div>'
                .htmlspecialchars($bak_estilo4, ENT_QUOTES).' <span class="rst-bak">(se eliminará al aplicar el reset)</span></div></div>';
        }
...
for (var _oi = 1; _oi <= 2; _oi++) { rstUpdateOficioSlot(_oi); }
        ...
        // Haki fields
        $haki_rows = '';
        // Los tres pueden ser -1 (no despertado) o 0 (despertado, nivel base). Un min más
        // alto que el valor real bloqueaba el envío del formulario (validación nativa del
        // navegador) y con ello la aprobación entera.
        $haki_fields = ['kenbun'=>'Kenbun (-1 a 8)','buso'=>'Buso (-1 a 8)','hao'=>'Hao (-1 a 8)','dominio_akuma'=>'Dominio Akuma (0-6)'];
        $haki_min = ['kenbun'=>-1,'buso'=>-1,'hao'=>-1,'dominio_akuma'=>0];
        $haki_max = ['kenbun'=>8,'buso'=>8,'hao'=>8,'dominio_akuma'=>6];
        ...
        // Antes truncada en tiers 1-10: le faltaban 11-15/SSS respecto a la
        // copia completa de op/personaje.php, así que una técnica de tier alto
        // se mostraba con 0h de coste al staff. Ahora coincide con esa.
        $tec_hours_r    = [1=>1,2=>4,3=>8,4=>12,5=>24,6=>36,7=>48,8=>60,9=>72,10=>100,11=>125,12=>150,13=>175,14=>200,15=>250,'SSS'=>300];
