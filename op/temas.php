<?php
/**
 * Tracker personal de temas de rol.
 */
...
if (!op_temas_tablas_listas()) {
    error('El tracker de temas todavia no esta instalado.');
}
...
if (!$opTemasFichaVista) {
    ...
    error('Necesitas una ficha para usar el tracker de temas de rol.');
}

function op_temas_nombre_personaje($row)
{
...
function op_temas_mensaje_resultado($code)
{
    $mensajes = array(
        'tema_agregado' => array('ok', 'El tema se agrego al tracker.'),
        ...
        'ya_seguido' => array('err', 'Ese tema ya esta en tu tracker.'),
...
function op_temas_form_attrs()
{
    return ' method="post" action="/op/temas.php" hx-post="/op/temas.php"'
        . ' hx-target="#op-temas-tracker" hx-swap="outerHTML"';
}

function op_temas_hidden_base($action, $seguimientoId = 0)
{
    global $mybb;
    $html = '<input type="hidden" name="my_post_key" value="' . op_temas_escape($mybb->post_code) . '">'
        . '<input type="hidden" name="action" value="' . op_temas_escape($action) . '">';
...
function op_temas_render_participante($participante, $seguimientoId, $removible)
{
    $uid = (int)$participante['participante_uid'];
    $nombre = op_temas_escape(op_temas_nombre_personaje($participante));
    ...
    if ($removible) {
        $accion = '<form class="op-temas-participante__accion"' . op_temas_form_attrs() . '>'
            . op_temas_hidden_base('retirar_participante', $seguimientoId)
    ...
    return '<div class="op-temas-participante op-temas-participante--' . $clase
        ...
        . '<img src="' . op_temas_escape($avatar) . '" alt="" loading="lazy" decoding="async">'
...
function op_temas_render_narrador($tema, $soloLectura = false)
{
    $id = (int)$tema['id'];
    ...
    if ($narradorUid > 0) {
        ...
        $actual = '<div class="op-temas-narrador__actual">'
            ...
            . '<a href="/op/personaje.php?uid=' . $narradorUid . '">' . op_temas_escape($nombre) . '</a>'
            . '<small>' . $estado . '</small></span>'
            . $quitar . '</div>';
    }

    if ($soloLectura) {
        return $actual === '' ? '' : '<section class="op-temas-narrador"><h4>Modo de ronda</h4>' . $actual . '</section>';
    }
    ...
    $buscador = '<form class="op-temas-agregar-participante op-temas-narrador__buscador"'
        . op_temas_form_attrs() . ' autocomplete="off">'
        . op_temas_hidden_base('establecer_narrador', $id)
        . '<div class="op-temas-buscador opg-buscar-caja">'
        . '<label for="narrador-buscar-' . $id . '">' . $etiqueta . '</label>'
        . '<input type="search" id="narrador-buscar-' . $id . '" name="q" placeholder="ID, nombre o apodo" aria-autocomplete="list"'
        . ' hx-get="/op/temas.php" hx-trigger="keyup changed delay:250ms" hx-target="next .op-temas-resultados"'
        . ' hx-vals=\'{"action":"buscar_personajes"}\'>'
        . '<input class="op-temas-participante-uid" type="hidden" name="narrador_uid" value="">'
        . '<div class="op-temas-resultados opg-resultados" role="listbox"></div></div>'
    ...
    return '<section class="op-temas-narrador"><h4>Modo de ronda</h4>' . $actual . $buscador . '</section>';
}

function op_temas_render_configuracion($tema, $soloLectura = false)
{
    $id = (int)$tema['id'];
    $respondieron = '';
    foreach ($tema['respondieron_lista'] as $participante) {
        $respondieron .= op_temas_render_participante($participante, $id, !$soloLectura);
    }
    if ($respondieron === '') {
    ...
    foreach ($tema['pendientes'] as $participante) {
        $pendientes .= op_temas_render_participante($participante, $id, !$soloLectura);
    }
    if ($pendientes === '') {
    ...
    if (!$soloLectura) {
        $turnos = '<div class="op-temas-turnos">'
            . '<form' . op_temas_form_attrs() . '>' . op_temas_hidden_base('me_toca', $id)
            ...
            . '<form' . op_temas_form_attrs() . '>' . op_temas_hidden_base('no_me_toca', $id)
    ...
    $buscador = '';
    if (!$soloLectura) {
        $buscador = '<form class="op-temas-agregar-participante"' . op_temas_form_attrs() . ' autocomplete="off">'
            . op_temas_hidden_base('agregar_participante', $id)
            . '<div class="op-temas-buscador opg-buscar-caja">'
            . '<label for="participante-buscar-' . $id . '">Agregar participante</label>'
            . '<input type="search" id="participante-buscar-' . $id . '" name="q" placeholder="ID, nombre o apodo" aria-autocomplete="list"'
            . ' hx-get="/op/temas.php" hx-trigger="keyup changed delay:250ms" hx-target="next .op-temas-resultados"'
            . ' hx-vals=\'{"action":"buscar_personajes"}\'>'
    ...
    return '<details class="op-temas-config">'
        ...
        . op_temas_render_narrador($tema, $soloLectura)
...
function op_temas_render_tarjeta($tema, $soloLectura = false)
{
    $id = (int)$tema['id'];
    $tid = (int)$tema['tid'];
    $titulo = op_temas_escape($tema['subject']);
    $foro = op_temas_escape($tema['forum_name']);
    $url = '/showthread.php?tid=' . $tid . '&action=lastpost';
    $ultimoNombre = op_temas_escape(op_temas_nombre_personaje(array(
    ...
    foreach ($tema['pendientes'] as $participante) {
        $pendientes .= '<a class="op-temas-pendiente" href="/op/personaje.php?uid=' . (int)$participante['participante_uid'] . '">'
            . op_temas_escape(op_temas_nombre_personaje($participante)) . '</a>';
    }
    if ($pendientes === '') {
    ...
    if (!empty($tema['narrador_uid'])) {
        ...
        $narradorNombre = op_temas_escape(op_temas_nombre_personaje($narrador));
    ...
    $configuracion = $cerrado ? '' : op_temas_render_configuracion($tema, $soloLectura);
    $retirar = '';
    if (!$soloLectura) {
        $retirar = '<form class="op-temas-retirar"' . op_temas_form_attrs() . ' onsubmit="return confirm(\'Confirmar: dejar de seguir este tema?\')">'
            . op_temas_hidden_base('dejar_seguir', $id)
    ...
    return '<article class="op-temas-card op-temas-card--' . $estadoClase . '">'
        ...
        . '<h3><a href="' . $url . '" target="_blank" rel="noopener">' . $titulo . '</a></h3></div>' . $retirar . '</header>'
        . '<div class="op-temas-card__meta"><span><i class="fa-solid fa-location-dot" aria-hidden="true"></i> ' . $foro . '</span>'
        . '<span>TID ' . $tid . '</span><span>Ultimo post: ' . $ultimoNombre . ' - ' . op_temas_escape($fecha) . '</span></div>'
        . ($cerrado ? '<p class="op-temas-cerrado-aviso"><i class="fa-solid fa-lock" aria-hidden="true"></i> Este tema esta cerrado.'
            . ($soloLectura ? '' : ' Retiralo del tracker cuando hayas terminado.') . '</p>'
            : '<div class="op-temas-ronda"><strong>' . $progreso . '</strong><div class="op-temas-pendientes">' . $pendientes . '</div></div>' . $configuracion)
        . '</article>';
}

function op_temas_render_resultado_busqueda($ficha)
{
    $uid = (int)$ficha['fid'];
    $label = op_temas_nombre_personaje($ficha) . ' (#' . $uid . ')';
    return '<button type="button" class="opg-resultado op-temas-personaje-opcion" role="option"'
        . ' data-uid="' . $uid . '" data-label="' . op_temas_escape($label) . '">'
        . op_temas_escape($label) . '</button>';
}

function op_temas_render_resultado_vista($ficha)
{
    $uid = (int)$ficha['fid'];
    $nombre = op_temas_escape(op_temas_nombre_personaje($ficha));
    return '<a class="opg-resultado op-temas-vista-opcion" role="option"'
        . ' href="/op/temas.php?modo_vista=' . $uid . '">'
        . $nombre . '<span>FID #' . $uid . '</span></a>';
}

function op_temas_render_alta()
{
    return '<form class="op-temas-alta"' . op_temas_form_attrs() . '>'
        . op_temas_hidden_base('agregar_tema')
...
function op_temas_render_tracker(
    $listado,
    $resultadoCode = '',
    $soloLectura = false,
    $vistaUid = 0,
...
)
{
    ...
    $op_temas_conteo_cerrados = (int)$listado['conteos']['cerrados'];

    $op_temas_lista_debes = '';
    foreach ($listado['debes_responder'] as $tema) {
        $op_temas_lista_debes .= op_temas_render_tarjeta($tema, $soloLectura);
    }
    if ($op_temas_lista_debes === '') {
        $op_temas_lista_debes = '<p class="op-temas-vacio"><i class="fa-solid fa-check" aria-hidden="true"></i> '
            . ($soloLectura ? 'Este personaje no debe ninguna respuesta.' : 'No debes ninguna respuesta.') . '</p>';
    }

    $op_temas_lista_esperando = '';
    foreach ($listado['esperando'] as $tema) {
        $op_temas_lista_esperando .= op_temas_render_tarjeta($tema, $soloLectura);
    }
    if ($op_temas_lista_esperando === '') {
        $op_temas_lista_esperando = '<p class="op-temas-vacio">'
            . ($soloLectura ? 'Este personaje no esta esperando respuestas.' : 'No estas esperando respuestas en ningun tema.') . '</p>';
    }

    $op_temas_lista_cerrados = '';
    foreach ($listado['cerrados'] as $tema) {
        $op_temas_lista_cerrados .= op_temas_render_tarjeta($tema, $soloLectura);
    }
    if ($op_temas_lista_cerrados === '') {
        $op_temas_lista_cerrados = '<p class="op-temas-vacio">'
            . ($soloLectura ? 'Este personaje no tiene temas cerrados en el tracker.' : 'No tienes temas cerrados pendientes de retirar.') . '</p>';
    }

    $op_temas_aviso = '';
    $mensaje = op_temas_mensaje_resultado($resultadoCode);
    if ($mensaje) {
        $op_temas_aviso = '<p class="aviso ' . $mensaje[0] . '" role="status">' . op_temas_escape($mensaje[1]) . '</p>';
    }

    $op_temas_modo_vista = '';
    if ($soloLectura) {
        $volver = $mostrarMisTemas
            ? '<a class="btn-op btn-op--sm btn-op--secundario" href="/op/temas.php">'
        ...
        $op_temas_modo_vista = '<aside class="op-temas-vista" aria-label="Modo vista">'
            ...
            . '<strong>Modo vista</strong> Temas de <a href="/op/personaje.php?uid=' . (int)$vistaUid . '">'
            . op_temas_escape($vistaNombre) . '</a></span></span>' . $volver . '</aside>';
    }
    $op_temas_alta = $soloLectura ? '' : op_temas_render_alta();
    $op_temas_post_key = op_temas_escape($mybb->post_code);
    eval("\$html = \"" . op_temas_cargar_plantilla('op_temas_tracker') . "\";");
    return $html;
}
...
if ($action === 'buscar_modo_vista') {
    ...
    $query = $db->query("SELECT fid,nombre,apodo
        FROM " . TABLE_PREFIX . "op_fichas
        WHERE fid != {$opTemasOwnerUid}
          AND (nombre LIKE '%{$like}%' OR apodo LIKE '%{$like}%' {$idWhere})
        ORDER BY CASE WHEN fid={$exactId} THEN 0
                      WHEN nombre='{$exact}' OR apodo='{$exact}' THEN 1 ELSE 2 END,
                 nombre ASC
        LIMIT 12");
    $html = '';
    while ($ficha = $db->fetch_array($query)) {
        $html .= op_temas_render_resultado_vista($ficha);
    }
    if ($html === '') {
...
if ($action === 'buscar_personajes') {
    ...
    $query = $db->query("SELECT fid,nombre,apodo
        ...
        WHERE fid != {$opTemasOwnerUid}
          AND (nombre LIKE '%{$like}%' OR apodo LIKE '%{$like}%' {$idWhere})
        ORDER BY CASE WHEN nombre='{$exact}' OR apodo='{$exact}' THEN 0 ELSE 1 END,
                 nombre ASC
        LIMIT 12");
    $html = '';
    while ($ficha = $db->fetch_array($query)) {
        $html .= op_temas_render_resultado_busqueda($ficha);
    }
    if ($html === '') {
        $html = '<p class="opg-resultado-vacio">No se encontraron personajes.</p>';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_post_check($mybb->get_input('my_post_key', MyBB::INPUT_STRING), true)) {
        ...
        switch ($action) {
            case 'agregar_tema':
                $resultado = op_temas_agregar_tema(
                    $opTemasOwnerUid,
                ...
                break;
            case 'dejar_seguir':
                $resultado = op_temas_dejar_seguir($opTemasOwnerUid, $seguimientoId);
                break;
            case 'me_toca':
                $resultado = op_temas_marcar_me_toca($opTemasOwnerUid, $seguimientoId);
                break;
            case 'no_me_toca':
                $resultado = op_temas_marcar_no_me_toca($opTemasOwnerUid, $seguimientoId);
                break;
            case 'agregar_participante':
                $resultado = op_temas_agregar_participante(
                    $opTemasOwnerUid,
                    $seguimientoId,
            ...
            case 'retirar_participante':
                $resultado = op_temas_retirar_participante(
                    $opTemasOwnerUid,
                    $seguimientoId,
                    $mybb->get_input('participante_uid', MyBB::INPUT_INT)
                );
            ...
            case 'establecer_narrador':
                $resultado = op_temas_establecer_narrador(
                    $opTemasOwnerUid,
                    $seguimientoId,
            ...
            case 'quitar_narrador':
                $resultado = op_temas_quitar_narrador($opTemasOwnerUid, $seguimientoId);
                break;
            default:
    ...
    if (!empty($_SERVER['HTTP_HX_REQUEST'])) {
        $listadoActualizado = op_temas_listar($opTemasOwnerUid);
        header('Content-Type: text/html; charset=utf-8');
        echo op_temas_render_tracker($listadoActualizado, $resultadoCode);
        echo op_temas_render_header($opTemasOwnerUid, $listadoActualizado['conteos']);
        exit;
    }
    header('Location: /op/temas.php?resultado=' . rawurlencode($resultadoCode));
    exit;
}

$listado = op_temas_listar($opTemasOwnerUid);
$opTemasVistaNombre = op_temas_nombre_personaje(array(
...
$opTemasMostrarMisTemas = $opTemasSessionUid > 0
    && op_temas_personaje_tiene_ficha($opTemasSessionUid);
$op_temas_tracker = op_temas_render_tracker(
    $listado,
...
if ($mybb->get_input('fragmento', MyBB::INPUT_STRING) === 'tracker') {
    header('Content-Type: text/html; charset=utf-8');
    echo $op_temas_tracker;
    exit;
}

$op_temas_titulo = $opTemasSoloLectura
    ? 'Temas de ' . op_temas_escape($opTemasVistaNombre)
    : 'Mis temas de rol';
$op_temas_subtitulo = $opTemasSoloLectura
    ? 'Vista publica de sus rondas de rol.'
    : 'Rondas activas del personaje actual.';
$op_temas_html_title = $opTemasSoloLectura
    ? op_temas_escape('Temas de ' . $opTemasVistaNombre)
    : 'Mis temas de rol';
$op_temas_vista_uid = (int)$opTemasOwnerUid;
$opTemasBreadcrumbUrl = $opTemasSoloLectura
    ? '/op/temas.php?modo_vista=' . $opTemasOwnerUid
    : '/op/temas.php';
add_breadcrumb($op_temas_titulo, $opTemasBreadcrumbUrl);
eval("\$page = \"" . op_temas_cargar_plantilla('op_temas') . "\";");
output_page($page);
