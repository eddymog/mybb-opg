...
/**
 * Adds a list of users attached to the account in header, sidebar
 * And below the quick reply, new reply and new thread editor.
...
{
    global $db, $mybb, $lang, $eas, $templates, $theme, $current_page, $as_header, $user_sec_reason;
    global $as_header_userbit, $as_sidebar, $as_sidebar_userbit, $as_header_dropdown, $as_post;
    global $as_post_userbit, $pm_switch_notice, $privatemessage_bit, $privatemessage_switch, $mybbUsername;
    global $mybbUid, $attachedPostUser, $userUid, $attachedUser, $userAvatar, $user_profilefield;
    ...
    if ($mybb->user['uid'] != 0) {
        ...
        $as_header = $as_header_userbit = $as_sidebar = $as_sidebar_userbit = $as_header_dropdown = '';
        $as_post_userbit = $privatemessage_bit = $privatemessage_switch = $pm_switch_notice = '';
        ...
        if ($mybb->usergroup['as_canswitch'] == 1 && $count > 0) {
            ...
            // Sort accounts by first, secondary, shared accounts and by uid or username
                ...
                foreach ($accounts as $key => $account) {
                    ...
                    if ($account['as_uid'] == $mybb->user['uid']) {
                        if ($count > 0) {
                            ...
                            if ($mybb->settings['aj_headerdropdown'] != 1) {
                                ...
                                $as_header_dropdown .= eval(
                                    $templates->render('accountswitcher_header_dropdown_userbit')
                                );
                            }
                            ...
                            if ($mybb->settings['aj_pmnotice'] == 1
                                && $account['pmnotice'] == 2
                                && $account['unreadpms'] > 0
                                && $mybb->settings['enablepms'] != 0
                                && ($current_page != "private.php" || $mybb->input['action'] != "read")
                            ) {
                                $privatemessage_bit = eval($templates->render('accountswitcher_newpm_messagebit'));
                                $privatemessage_switch .= $lang->sprintf(
                                    $lang->aj_newpm_switch_notice_one,
                                    $privatemessage_bit
                                );
                    ...
                    if ($count > 0
                        && $mybb->settings['aj_shareuser'] == 1
                        && (($account['as_share'] != 0
                            && $account['as_shareuid'] == 0
                            && $account['as_buddyshare'] == 0)
                        || ($account['as_shareuid'] != 0
                            && $account['as_shareuid'] == $mybb->user['uid'])
                        || ($account['as_share'] != 0
                            && $account['as_shareuid'] == 0
                            && $account['as_buddyshare'] != 0
                            && !empty($buddylist)
                            && in_array($mybb->user['uid'], $buddylist))
                        )
                    ) {
                    // Set username styles
                        if ($mybb->settings['aj_sharestyle'] == 1 && $account['as_share'] != 0) {
            ...
            // Create link to the master
                ...
                // Sort accounts by first, secondary, shared accounts and by uid or username
                    ...
                    foreach ($accounts as $key => $account) {
                        ...
                        if ($mybb->settings['aj_shareuser'] == 1
                            && (($account['as_share'] != 0
                                && $account['as_shareuid'] == 0
                                && $account['as_buddyshare'] == 0
                                )
        ...
        // If there are no users attached to the current account and
        // The current account is not attached to another user
        if ($count == 0
            && $mybb->user['as_uid'] == 0
            && $mybb->user['as_share'] == 0
        ...
        if ($mybb->settings['aj_shareuser'] == 1
            ...
            if ($mybb->settings['aj_sharestyle'] == 1 && $attachedShare['as_share'] != 0) {
                ...
                && $attachedShare['as_sec'] != 0
                && $attachedShare['as_share'] == 0
            ) {
                $user_sec_reason = htmlspecialchars_uni($account['as_secreason']);
                $attachedUser = eval($templates->render('accountswitcher_sec_accountsbit'));
            } else {
                $attachedUser = format_name(
                    $attachedPostUser,
                    (int)$attachedShare['usergroup'],
                    (int)$attachedShare['displaygroup']
                );
            }
            // Load userbits
            if ($mybb->settings['aj_headerdropdown'] != 1) {
    ...
    // Sidebar enabled
    if (isset($mybb->settings['aj_sidebar'])
        && $mybb->settings['aj_sidebar'] == 1
        && trim($as_sidebar_userbit) != ''
    ) {
        $as_sidebar = eval($templates->render('accountswitcher_sidebar'));
    }
    // Header dropdown selected
    if (isset($mybb->settings['aj_headerdropdown'])
        && $mybb->settings['aj_headerdropdown'] == 1
        && trim($as_header_dropdown) != ''
    ) {
...
{
    global $mybb, $pid, $tid, $post, $db, $theme, $eas, $headerinclude, $lang, $templates, $postlink;
    global $userUid, $attachedUser, $as_author_userbit, $cancel;

    // If user author change or mod author change
    if (($mybb->input['changeauthor'] == 1 && $mybb->settings['aj_changeauthor'] == 1)
        || ($mybb->input['adminauthor'] == 1 && $mybb->settings['aj_admin_changeauthor'] == 1)
    ) {
        ...
        // Get the number of users attached to this account
            ...
            // Search und set new author
                $lang->load('global');
                $author_admin .= '<div class="modal">'.
                                    eval($templates->render('accountswitcher_author_admin'))
                                    .'</div>';
            ...
            elseif ($mybb->input['changeauthor'] == 1) {
                ...
                // If there are no users attached to current account
                // But the current account is attached to another user
                if ($count == 0 && $mybb->user['as_uid'] != 0) {
                // Get the master
                    $master = get_user($mybb->user['as_uid']);
...
function accountswitcher_author_change()
{
    ...
    // Change action
    if ($mybb->input['action'] == 'do_author'
        && $mybb->request_method == 'post'
        && ($mybb->settings['aj_changeauthor'] == 1
            || $mybb->settings['aj_admin_changeauthor'] == 1
        )
    ) {
    // Verify incoming POST request
        ...
        if ($pid == $thread['firstpost']
            && $forum['usethreadcounts'] != 0
            && substr($thread['closed'], 0, 6) != 'moved|'
        ) {
            if ($thread['visible'] == 1) {
        ...
        if ($db->update_query('posts', $updated_record, "pid='".(int)$post['pid']."'")) {
            ...
            $lang->aj_author_change_log = $lang->sprintf(
                $lang->aj_author_change_log,
                (int)$post['pid'],
                htmlspecialchars_uni($post['username']),
                htmlspecialchars_uni($newauthor['username'])
            );
            ...
            if ($post['uid'] != $mybb->user['uid'] && $mybb->settings['aj_admin_changeauthor'] == 1) {
                if ($mybb->settings['aj_authorpm'] == 1) {
                    ...
                    $lang->aj_author_change_pm_body = $lang->sprintf(
                        $lang->aj_author_change_pm_body,
                        htmlspecialchars_uni($mybb->user['username']),
                        $mybb->settings['bburl'].'/'.htmlentities($_POST['p_link']),
...
{
    global $mybb, $theme, $lang, $admin_change, $author_change, $templates, $eas;
    ...
    if (($mybb->user['uid'] != 0
            ...
            && ($count > 0 || $mybb->user['as_uid'] != 0)
            && $mybb->settings['aj_changeauthor'] == 1
        )
        || ($mybb->settings['aj_admin_changeauthor'] == 1
            && $mybb->settings['aj_admin_changegroup'] == 'admin'
            && $mybb->usergroup['cancp'] == 1
        )
        || ($mybb->settings['aj_admin_changeauthor'] == 1
            && $mybb->settings['aj_admin_changegroup'] == 'supermods'
            && ($mybb->usergroup['issupermod'] == 1)
        )
        || ($mybb->settings['aj_admin_changeauthor'] == 1
            && $mybb->settings['aj_admin_changegroup'] == 'mods'
            && is_moderator($post['fid'])
        )
    ) {
    // Declare variables
        ...
        if (!is_moderator($post['fid']) && $mybb->usergroup['edittimelimit'] != 0 && (TIME_NOW > $editlimit)) {
            return;
        }
...
function accountswitcher_profile()
{
    ...
    // Get usergroup permissions
        ...
        $as_postcount = (int)str_replace($mybb->settings['thousandssep'], '', $memprofile['postnum']);
        ...
        if ($mybb->usergroup['cancp'] != 1
            ...
            && $memprofile['as_privacy'] == 1
        ) {
            if (($mybb->user['as_uid'] != 0
                && $mybb->user['as_uid'] != $memprofile['as_uid']
                && $mybb->user['as_uid'] != $memprofile['uid']
            )
            || ($mybb->user['as_uid'] == 0
                && $mybb->user['uid'] != $memprofile['as_uid'])
            ) {
                return;
            }
        }

        // Get the number of attached accounts
        $numattached = 0;
        if ($count > 0) { // For master
            $numattached = $count;
        } elseif ($memprofile['as_uid'] != 0) { // For attached
            $numattached = $eas->get_attached($memprofile['as_uid']);
        }

        // If there are users attached and the current user can use the Enhanced Account Switcher...
        if ($permissions['as_canswitch'] == 1 && $count > 0) {
            $as_postcount += $eas->get_postcount($memprofile['uid']);
        ...
        // Get the master
            ...
            if ($permission['as_canswitch'] == 1) {
                ...
                $attachedPostUser = htmlspecialchars_uni($master['username']);
                $as_postcount = $eas->get_postcount($memprofile['as_uid']) + (int)$master['postnum'];
                $userAvatar = $eas->attached_avatar($master['avatar'], $master['avatardimensions']);
                ...
                if ($memprofile['uid'] == $mybb->user['uid']) {
                    ...
                    if (($mybb->usergroup['cancp'] != 1
                        ...
                        $attachedUser = '<span style="font-weight: bold;" title="Master Account">'.
                            ...
                            .'</span>';
                        $userAvatar = '<a href="'.$mybb->settings['bburl'].'/'.get_profile_link($userUid).'">'.
                                        $userAvatar.'</a>';
        ...
        if ($count > 0 || $count == 0 && $memprofile['as_uid'] != 0) {
            ...
            if ($memprofile['uid'] == $mybb->user['uid']
                || $memprofile['uid'] == $mybb->user['as_uid']
                || $memprofile['as_uid'] == $mybb->user['uid']
            ) {
                $hidden = 0;
            }
            if ($hidden > 0) {
        ...
        $as_postcount = my_number_format($as_postcount);
        $attached_post_count = eval($templates->render('accountswitcher_attached_postcount'));
    }
}
...
{
    global $mybb, $db, $theme, $lang, $eas, $as_postuser_userbit, $as_postuser_hidden, $user_sec_reason;
    global $attachedUser, $userUid, $masterUid, $postId, $templates, $attachedPostUser, $numaccounts;
    global $as_postcount, $user_profilefield, $templates;
    ...
    // Get usergroup permissions
        ...
        if ($mybb->usergroup['cancp'] != 1
            ...
            if (($mybb->user['as_uid'] != 0
                    && $mybb->user['as_uid'] != $post['as_uid']
                    && $mybb->user['as_uid'] != $post['uid']
                )
                || ($mybb->user['as_uid'] == 0
                    && $mybb->user['uid'] != $post['as_uid']
                )
            ) {
                return;
            }
        ...
        // If there are users attached and the current user can use the Enhanced Account Switcher...
        if ($permissions['as_canswitch'] == 1 && $count > 0) {
            $as_postcount += $eas->get_postcount($post['uid']);
        ...
        // Get the master
            ...
            if ($permission['as_canswitch'] == 1) {
                $userUid = $masterUid = (int)$master['uid'];
                $attachedPostUser = htmlspecialchars_uni($master['username']);
                $as_postcount = $eas->get_postcount($post['as_uid']) + (int)$master['postnum'];
                $userAvatar = $eas->attached_avatar($master['avatar'], $master['avatardimensions']);
                // Create link to master
                if ($post['uid'] == $mybb->user['uid']) {
                    ...
                    $post['masteraccount'] = $attachedUser;
                } else {
                    // Hide users with privacy setting enabled
                    if (($mybb->usergroup['cancp'] != 1
                            && $mybb->user['uid'] != $master['uid']
        ...
        if ($count > 0 || $count == 0 && $post['as_uid'] != 0) {
            ...
            // Show hidden accounts if current user is post author or attached to hidden account
            if ($post['uid'] == $mybb->user['uid']
                || $post['uid'] == $mybb->user['as_uid']
                || $post['as_uid'] == $mybb->user['uid']
            ) {
                $hidden = 0;
            }
            // Accountlist link in postbit
            if ($post['uid'] != 0 && $mybb->settings['aj_postuser'] == 1) {
                if ($count == 1) {
                    $numaccounts = $count;
            ...
            if ($mybb->settings['aj_postcount'] == 1 && $post['uid'] != 0) {
                $as_postcount = my_number_format($as_postcount);
                $attached_post_count = eval($templates->render('accountswitcher_attached_postcount'));
        ...
        if ($post['as_share'] != 0) {
            $post['attached_accounts'] .= nl2br("\n".$lang->aj_memberlist_shared);
        }
    }
}
...
{
    global $mybb, $db, $theme, $lang, $eas, $as_user_userbit, $as_user_hidden, $user_sec_reason, $as_postcount;
    global $attachedUser, $userUid, $masterUid, $templates, $attachedPostUser, $numaccounts, $user_profilefield;
    global $attached_post_count, $templates, $masteraccount;
    ...
    $mybb_asset_url = $mybb->asset_url;
    $as_postcount = (int)str_replace($mybb->settings['thousandssep'], '', $user['postnum']);

    // Get the attached users
    if ($mybb->settings['aj_memberlist'] == 1) {
    // Get usergroup permissions
        ...
        if ($permissions['as_canswitch'] == 1 && $count > 0) {
            ...
            $as_postcount += $eas->get_postcount($user['uid']);
        }
        ...
        // Get the master
            ...
            if ($permission['as_canswitch'] == 1) {
                $userUid = $masterUid = (int)$master['uid'];
                $attachedPostUser = htmlspecialchars_uni($master['username']);
                $as_postcount = $eas->get_postcount($user['as_uid']) + (int)$master['postnum'];
                $userAvatar = $eas->attached_avatar($master['avatar'], $master['avatardimensions']);
...
{
    global $mybb, $footer, $headerinclude, $header, $eas, $lang, $templates, $mybbUid, $mybbUsername;
    global $as_postreply, $as_canswitch, $as_reload, $as_script, $as_footer;
...
/**
 * The switch function deletes the mybbuser cookie,
 * Sets a new cookie for the selected account and starts a new session.
...
function accountswitcher_switch()
{
    ...
    if ($mybb->user['uid'] != 0
        && isset($mybb->input['switchuser'])
        && $mybb->input['switchuser'] == 1
        && $mybb->request_method == 'post'
    ) {
        ...
        if ($userPermission['as_canswitch'] == 1 || $masterPermission['as_canswitch'] == 1) {
            ...
            // Make sure you can switch to an attached account only
                // from master to an attached account
            if ($user['as_uid'] == $mybb->user['uid']
                // From attached to another attached account - both must have the same master
                || ($user['as_uid'] != 0 && $user['as_uid'] == $mybb->user['as_uid'])
                // From attached to the master account
                || $user['uid'] == $mybb->user['as_uid']
                // To a free shared account
                || $user['as_shareuid'] == $mybb->user['uid']
                // Return from a shared account to the account you switched from
                || $user['uid'] == $mybb->user['as_shareuid']
            ) {
                ...
                if ($mybb->user['uid']) {
                    ...
                    $db->shutdown_query(
                        "UPDATE ".TABLE_PREFIX."users SET lastvisit='{$time}',
                        lastactive='{$time}' WHERE uid='{$mybb->user['uid']}'"
                    );
                ...
                echo json_encode(array('postcode' => $mybb->post_code));
                exit;
            } else {
                log_moderator_action(
                    array(
...
function accountswitcher_clear_shared()
{
    ...
    $shareduser = get_user_by_username($mybb->get_input('username'), array('fields' => '*'));
    if ($shareduser['uid'] != 0) {
...
// Hook for correcting the modal z-index
$plugins->add_hook('pre_output_page', 'accountswitcher_output', -1000);
/**
 * Change z-index of MyBB's modal box to work with select2 autocomplete box.
 *
 */
function accountswitcher_output($page)
{
    /**
    *
    * Replace the code
    *
    **/
    $page = str_replace("var modal_zindex = 9999;", "", $page);
    $page = str_replace("var modal_zindex = 9995;", "", $page);
    $page = str_replace("var cookieDomain", "var modal_zindex = 9995;\n	var cookieDomain", $page);
    return $page;
}

// Hook for a registered new user
$plugins->add_hook('member_do_register_end', 'accountswitcher_register_attach');
/**
 * Redirect an new registered user to attach his account if setting is enabled and
 * there exists already a master account with the same email address
 *
 */
function accountswitcher_register_attach()
{
    global $mybb, $db, $lang, $eas, $user_info;

    if (isset($mybb->settings['aj_regmailattach']) && $mybb->settings['aj_regmailattach'] == 1) {
        ...
        if (count($masters) > 0) {
            if (!isset($lang->as_redirect_register)) {
                $lang->load('accountswitcher');
            }
                redirect("usercp.php?action=as_registerattach", $lang->as_redirect_register, "", true);
        }
    }
}
