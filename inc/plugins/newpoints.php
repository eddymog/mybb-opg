...
function newpoints_addpoints($uid, $points, $forumrate = 1, $grouprate = 1, $isstring = false, $immediate = false)
{
	...
	$experiencia_limite_query = $db->query(" SELECT * FROM mybb_op_experiencia_limite WHERE uid='$uid' AND semana='$weekNumber'; ");
	...
	if ($doesLimitExistByWeek == false) {
		$db->query(" 
			INSERT INTO `mybb_op_experiencia_limite`(`uid`, `semana`, `experiencia_semanal`) VALUES ('$uid','$weekNumber','$experienciaSemanal')
		");
	}
	...
	$pointsAwarded = $pointsAwarded - $minusExperiencia;
	$pointsAwarded = (abs($pointsAwarded) < 0.01) ? 0 : $pointsAwarded;
	
	$db->query(" 
		UPDATE `mybb_op_experiencia_limite` SET `experiencia_semanal`='$nuevaExperienciaSemanal' WHERE uid='$uid' AND semana='$weekNumber';
	");

	$subject = addslashes($thread['subject']);
	$threadid = $thread['tid'];
	
	$exp_combinada = $exp_actual + $pointsAwarded;
	$kuro_combinado = $kuros_actual + $kuroExperiencia;

	$log_expe = "$pointsAwarded de Experiencia ($exp_actual + $pointsAwarded = $exp_combinada) y $kuroExperiencia Kuros ($kuros_actual + $kuroExperiencia = $kuro_combinado). [$threadid ||| $subject]";
	$db->query("
        INSERT INTO `mybb_op_audit_general`(`uid`, `username`, `user_uid`, `categoria`, `log`) VALUES ('$uid','$user_username','$uid', '[Post]', '$log_expe');
    ");
	echo("<br><br><div style='text-align: center;'><h3>¡Has ganado $pointsAwarded de Experiencia y $kuroExperiencia Kuros!</h3></div>");
	...
	if ($isstring) // where username
		$db->write_query("UPDATE ".TABLE_PREFIX."users SET newpoints=newpoints+'". $pointsAwarded ."' WHERE username='".$db->escape_string($uid)."'");
	else // where uid
	{
		// if immediate, run the query now otherwise add it to shutdown to avoid slow down
		if ($immediate)
			$db->write_query("UPDATE ".TABLE_PREFIX."users SET newpoints=newpoints+'". $pointsAwarded ."' WHERE uid='".intval($uid)."'");
		else
		{
			$userpoints[intval($uid)] += $pointsAwarded;
		}
	}
