...
class errorHandler {
	...
	function error($type, $message, $file=null, $line=0, $allow_output=true)
	{
		...
		// When the debug helper is active, always log regardless of forum settings
		elseif(defined("MYBB_DEBUG_LOG") && MYBB_DEBUG_LOG)
		{
			$this->log_error($type, $message, $file, $line);
		}
	...
	function log_error($type, $message, $file, $line)
	{
		...
		if($type == MYBB_SQL)
		{
			$sql_message = "SQL Error: {$message['error_no']} - {$message['error']}\nQuery: {$message['query']}";
		}

		// Do not log something that might be executable
		$sql_message = str_replace('<?', '< ?', $sql_message);
		...
		// ── Extra request context added to the XML log ────────────────────────
		$req_method  = isset($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : 'CLI';
		$req_uri     = isset($_SERVER['REQUEST_URI'])    ? htmlspecialchars($_SERVER['REQUEST_URI'], ENT_XML1) : 'N/A';
		$req_ip      = 'N/A';
		foreach(array('HTTP_CF_CONNECTING_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_CLIENT_IP', 'REMOTE_ADDR') as $_ipkey)
		{
			if(!empty($_SERVER[$_ipkey]))
			{
				$_ip = trim(explode(',', $_SERVER[$_ipkey])[0]);
				if(filter_var($_ip, FILTER_VALIDATE_IP)) { $req_ip = $_ip; break; }
			}
		}
		$req_ua      = isset($_SERVER['HTTP_USER_AGENT']) ? htmlspecialchars($_SERVER['HTTP_USER_AGENT'], ENT_XML1) : 'N/A';
		$req_memory  = function_exists('memory_get_usage') ? round(memory_get_usage(true) / 1024) . 'KB' : 'N/A';

		$error_data  = "<error>\n";
		$error_data .= "\t<dateline>".TIME_NOW." (".date('Y-m-d H:i:s', TIME_NOW).")</dateline>\n";
		...
		$error_data .= "\t<message>".$sql_message."</message>\n";
		$error_data .= "\t<request>{$req_method} {$req_uri}</request>\n";
		$error_data .= "\t<ip>{$req_ip}</ip>\n";
		$error_data .= "\t<ua>{$req_ua}</ua>\n";
		$error_data .= "\t<memory>{$req_memory}</memory>\n";
