...
$templatelist .= ",rtchat_chat,op_index";
...
$op_functions_path = dirname(__FILE__) . '/../functions/op_functions.php';
if (file_exists($op_functions_path)) {
    require_once $op_functions_path;
    
    // Solo aplicar rate limiting si las funciones están disponibles
    if (function_exists('get_client_ip') && function_exists('check_rate_limit') && function_exists('log_security_event')) {
        $identifier = isset($mybb->user['uid']) && $mybb->user['uid'] > 0 
            ? $mybb->user['uid'] 
            : get_client_ip();

        // Límite para index: 150 peticiones por minuto
        if (check_rate_limit($identifier, 150, 60)) {
            log_security_event(
                'rate_limit_exceeded', 
                'index.php',
                array(
            ...
            echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Límite de peticiones excedido</title></head><body>';
            echo '<h1>⚠️ Demasiadas peticiones</h1>';
            echo '<p>Has excedido el límite de peticiones permitidas. Por favor, espera un momento antes de volver a cargar la página.</p>';
            echo '<p>Este límite existe para proteger el servidor de ataques y sobrecarga.</p>';
            echo '<p><a href="javascript:history.back()">Volver</a></p>';
            echo '</body></html>';
            exit;
        }

        // Registrar acceso al index (solo 1 de cada 10 para no saturar)
        if (rand(1, 10) == 1) {
            log_security_event('index_access', 'index.php', array('sample' => '1/10'));
        }
    }
}
